...
void ObjectData::ListWorkpace()
{
    ...
    QSqlQuery q;
    q.prepare("select w.workplace_id, w.version_type_id, t.version_name, w.pos_id, w.ipadr, w.vnc_port, w.vnc_pass, w.terminal_id from workplaces w "
    ...
    while(q.next()){
        ...
        wk.setPassVNC(q.value(6).toString());
        wk.setTerminalID(q.value(7).toInt());
